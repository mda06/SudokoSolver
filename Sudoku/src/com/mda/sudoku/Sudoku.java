...
public class Sudoku {
	...
	public void initGrid() {
		...
		/*for(int i = 0; i < 30; i++) {
			int x = rand.nextInt(grid.length), y = rand.nextInt(grid[0].length), v = 1 + rand.nextInt(9);
			if(isValid(v, x, y)) put(v, x, y);
			else i--;
		}*/
	...
	public void print() {
		System.out.print("    ");
		for(int i = 0; i < grid.length; i++) System.out.print(i + " ");
		System.out.println();
		System.out.print("    ");
		for(int i = 0; i < grid.length; i++) System.out.print("! ");
		System.out.println();
	...
	public void put(int value, int x, int y) {
		if(isValid(value, x, y)) grid[x][y] = value;
	}
	
	public boolean isValid(int value, int x, int y) {
		if(!isValidRow(value, y) || !isValidCol(value, x) || !isValidBlock(value, x, y)) return false;
		return true;
	}
	
	public boolean isValidBlock(int value, int x, int y) {
		if(!checkBounds(x, y) || !isEmpty(x, y)) return false;
		
		int bx = x / 3, by = y / 3;

		for(int i = 0; i < 3; i++) {
			for(int j = 0; j < 3; j++) {
				if(grid[i + bx * 3][j + by * 3] == value) return false;
			}
		}
